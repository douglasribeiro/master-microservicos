 ...
 * 
 * .route(p -> p.path("/get") .filters(f -> f.addRequestHeader("Chave", "Valor")
 * .addRequestParameter("Chave-p", "valor-p")) .uri("http://httpbin.org:80"))
 * 
 * .route(p -> p.path("/book-service/**").uri("lb://book-service")) .route(p ->
 * p.path("/exchange-service/**").uri("lb://exchange-service")) .build();
 * 
 * }
 * 
 * }
